...
-- for polyglot standard - most used in chess engines
...
function binNumb(s,p0,plen)
  local r=0;
  local b=1;
  local i=p0+plen-1;
  if( s~=nil and i<string.len(s) ) then
	while i>=p0 do
...
-- this reads all positions into memory
...
-- generates key for current position
function getKey()
 local k = i64(0);
 local pc="pNBRQK";
 local i = 1;
 ...
 while i<string.len(s) do
   local col = string.sub(s,i,i);
   local piece = string.sub(s,i+1,i+1);
   local at = string.sub(s,i+2,i+3);
   h = string.byte(at,1)-97;
   v = string.byte(at,2)-49;
   i = i+5;
   x=(string.find(pc,piece)-1) * 2;
   if col=="w" then
     x=x+1;
   end
   k = i64_xor( k, Random64[1+(64*x)+(8*v)+h] );
 end

 -- castling
 if( not (c0_LuaChess.c0_w00 or c0_LuaChess.c0_wKingmoved )
		and not c0_LuaChess.c0_wLRockmoved ) then
    k = i64_xor( k, Random64[1+769] );
 end
 if( not (c0_LuaChess.c0_w00 or c0_LuaChess.c0_wKingmoved )
		and not c0_LuaChess.c0_wRRockmoved ) then
    k = i64_xor( k, Random64[1+768] );
 end
 if( not (c0_LuaChess.c0_b00 or c0_LuaChess.c0_bKingmoved )
		and not c0_LuaChess.c0_bLRockmoved ) then
    k = i64_xor( k, Random64[1+771] );
 end
 if( not (c0_LuaChess.c0_b00 or c0_LuaChess.c0_bKingmoved )
		and not c0_LuaChess.c0_bRRockmoved ) then
    k = i64_xor( k, Random64[1+770] );
 end
 ...
 if c0_LuaChess.c0_sidemoves > 0 then
    k = i64_xor( k, Random64[1+780] );
 end

 return i64_unsigned(k);
end


-- .bin file should be ascending ordered (for binary search)
-- returns array element for key k
...
-- returns position weight after lookup
function getWeight(i)
 return binBase[i].weight;
end
...
while(j~=nil) do
  ...
  local weight = getWeight(j);
  print( "Move:" .. move .. " weight:" .. string.format("%d",weight) );
  j=j+1;
  if( j>table.getn(binBase) or i64_ne( key, binBase[j].key )) then
	j=nil;
  end
end
